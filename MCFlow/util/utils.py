...
def endtoend_test(flow, nn_model, data_loader, args):
    ...
    for index, (vectors, labels) in enumerate(data_loader):
        ...
        imp = torch.clamp(x_hat, min=0, max=1)
        imp = x_hat

        # print("imp",imp)
        # print("real",labels[0]) 
        batch_loss = torch.sum(loss(imp, labels[0]) * (1-labels[1]))
        total_imputing += np.sum((1-labels[1]).cpu().numpy())
        imp = labels[0] * labels[1] + imp * (1-labels[1])
        # print("mask",labels[1])
        # print("imp",imp)
        # print("leave",imp * (1-labels[1]))

        totalloss+=batch_loss.item()

        imputed_total.append(imp)

    index+=1
    return totalloss/total_imputing, nf_totalloss/index,imputed_total
...
def inference_img_imputation_networks(nn, nf, data, mask, original_dat, args):
    ...
    ones = np.ones((64, data[0].shape[0]))
